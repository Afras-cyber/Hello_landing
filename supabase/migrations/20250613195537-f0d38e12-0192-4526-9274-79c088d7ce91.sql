...
-- Fix Finnish month name parsing in analyze_screenshot_conversion function
...
BEGIN
  ...
  -- Parse Finnish date to proper timestamp
  IF appointment_date_text IS NOT NULL THEN
    BEGIN
      -- Convert Finnish month names to numbers
      appointment_date_text := regexp_replace(appointment_date_text, 'tammikuuta', '01', 'gi');
      appointment_date_text := regexp_replace(appointment_date_text, 'helmikuuta', '02', 'gi');
      appointment_date_text := regexp_replace(appointment_date_text, 'maaliskuuta', '03', 'gi');
      appointment_date_text := regexp_replace(appointment_date_text, 'huhtikuuta', '04', 'gi');
      appointment_date_text := regexp_replace(appointment_date_text, 'toukokuuta', '05', 'gi');
      appointment_date_text := regexp_replace(appointment_date_text, 'kesäkuuta', '06', 'gi');
      appointment_date_text := regexp_replace(appointment_date_text, 'heinäkuuta', '07', 'gi');
      appointment_date_text := regexp_replace(appointment_date_text, 'elokuuta', '08', 'gi');
      appointment_date_text := regexp_replace(appointment_date_text, 'syyskuuta', '09', 'gi');
      appointment_date_text := regexp_replace(appointment_date_text, 'lokakuuta', '10', 'gi');
      appointment_date_text := regexp_replace(appointment_date_text, 'marraskuuta', '11', 'gi');
      appointment_date_text := regexp_replace(appointment_date_text, 'joulukuuta', '12', 'gi');
      
      -- Convert to DD.MM.YYYY format
      appointment_date_text := regexp_replace(appointment_date_text, '(\d{1,2})\.\s*(\d{2})\s*(\d{4})', '\1.\2.\3', 'g');
      
      -- Parse the date
      parsed_date := to_date(appointment_date_text, 'DD.MM.YYYY')::timestamp with time zone;
    EXCEPTION
      WHEN OTHERS THEN
        parsed_date := NULL;
    END;
